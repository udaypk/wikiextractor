...
def process_dump(input_file, template_file, out_file, file_size, file_compress,
    ...
    redirect = False
    log_category=False
    filtered_namespaces=["Interplanetary magnetic field", "Interplanetary dust cloud", "Interstellar medium", "Intracluster medium", "Intergalactic media", "Life in space", "Space art", "Space lists", "Space stubs", "Cosmic dust", "Message artifacts", "Astropolitics", "Women in space", "Space exploration", "Outer space themed songs", "Interplanetary medium", "Outer space", "Space geodesy", "Space industry", "Industry in space", "Space in fiction", "Music in space", "Space law", "Space robots", "Works about outer space", "Works set in outer space", "Space technology", "Space applications", "Politics of outer space", "Spaceflight", "Astronomical objects", "Space force", "Outer space", "UFO sightings in outer space", "CondoSat", "Interstellar media", "Photographs of Earth from outer space", "Astronomy images", "Space photography and videography", "Satellites", "Space physics", "Space by decade", "Space by year", "Space by century", "People associated with outer space"]


    for line in input:
        #logging.info("Category line found: " )
        
        if (line[0:11]=='[[Category:' ):#and line[-2:]==']]'
            #logging.info("Category line found: "+ str(line) ) 
            for required_namespace in filtered_namespaces:  
                if required_namespace.lower() in line.lower():
                    log_category=True
                    logging.info("Found required category: "+ str(line) )
            for required_namespace in acceptedNamespaces:  
                if required_namespace.lower() in line.lower():
                    log_category=True
                    logging.info("Found required category: "+ str(line) )         
        ...
        if '<' not in line:  # faster than doing re.search()
            ...
            continue
        m = tagRE.search(line)
        #logging.info("Printing m : "+ str(m) )
        #logging.info("Printing m group 2: "+ str(m.group(2)) )
        #logging.info("Printing m group 3: "+ str(m.group(3)) )
        if not m:
            continue
        tag = m.group(2)
        if tag == 'page':
            ...
            colon = title.find(':')
            logging.info("Processing title: "+ title )
            if (log_category) and (colon < 0 or (title[:colon] in acceptedNamespaces) and id != last_id and
